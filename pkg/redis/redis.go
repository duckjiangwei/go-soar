...
//有序集合插入
func (rds RedisClient) Zadd(key string, score float64, member interface{}) bool {
	z := redis.Z{
		Score:  score,
		Member: member,
	}
	if err := rds.Client.ZAdd(rds.Context, key, &z).Err(); err != nil {
...
//从有序集合弹出
func (rds RedisClient) Zrem(key string, member interface{}) bool {
	if err := rds.Client.ZRem(rds.Context, key, member).Err(); err != nil {
...
//获取有序集合元素个数
func (rds RedisClient) Zcard(key string) int64 {
	result, err := rds.Client.ZCard(rds.Context, key).Result()
	if err != nil {
		if err != redis.Nil {
			logger.ErrorString("Redis", "ZCard", err.Error())
		}
		return 0
	}
	return result
...
func (rds RedisClient) Zrange(key string, start, stop int64) (result []string) {
	result, err := rds.Client.ZRange(rds.Context, key, start, stop).Result()
	if err != nil {
		if err != redis.Nil {
